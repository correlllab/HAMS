...
//       sub  /h12_deploy_mjpc/base_state  (nav_msgs/Odometry, from estimator_node)
...
//   * the base pose+velocity comes from the estimator's Odometry directly, so the
//     upstream IMU-site->pelvis back-out and the single-stream velocity finite-diff
//     are gone (and with them the twin_dt/use_twin_time machinery -- audit M6).
//
...
constexpr char kTaskId[] = "Stabilize H12 Magpie";  // lower-body nu=12 stabilize task
constexpr int kStrategy = 6;                          // stable stand (no live switch)
constexpr double kCtrlHz = 200.0;
constexpr double kWarmupSec = 1.0;
constexpr double kStartRampSec = 5.0;   // measured -> stance ramp on all leg joints
constexpr double kRampHoldSec = 3.0;    // hold the stance scripted while CEM converges
constexpr double kPolicyBlendSec = 2.0; // ease scripted stance -> live policy target
constexpr double kStaleSec = 0.05;      // H1 watchdog: state older than this -> safe-hold
constexpr float kSafeHoldKd = 2.0f;     // damping-stop kd on safe-hold (kp=0, tau=0)
...
class ControllerNode : public rclcpp::Node {
 public:
  ControllerNode() : rclcpp::Node("h12_deploy_mjpc_controller") {
    ...
    // Self-contained standing demo: once full-authority policy has held the torso
    // upright (tilt < band_stable_tilt_deg) for band_stable_secs, drop the sim's
    // elastic support tether via /elastic_band/toggle (once). Disable on the real
    // robot (no band); if the service is absent it simply warns and never fires.
    drop_band_ = declare_parameter<bool>("drop_band", true);
    band_stable_secs_ = declare_parameter<double>("band_stable_secs", 6.0);
    band_stable_tilt_ = declare_parameter<double>("band_stable_tilt_deg", 10.0) * M_PI / 180.0;
    const std::string lowstate_topic = declare_parameter<std::string>("lowstate_topic", "/lowstate");
    const std::string base_topic =
        declare_parameter<std::string>("base_state_topic", "/h12_deploy_mjpc/base_state");
    ...
    // Subscriptions run in a REENTRANT group so /lowstate + /base_state keep being
    ...
    base_sub_ = create_subscription<Odometry>(
        base_topic, qos, [this](Odometry::SharedPtr m) {
          std::lock_guard<std::mutex> lk(state_mu_);
          last_base_ = m;
          last_base_time_ = now();
        }, sub_opts);
 ...
 private:
  ...
  // Fill the planner state from the latest LowState (joints) + Odometry (pelvis).
  void FillState(const LowState& ls, const Odometry& od) {
    // pelvis world pose straight from the estimator (no IMU-site back-out).
    sd_->qpos[0] = od.pose.pose.position.x;
    sd_->qpos[1] = od.pose.pose.position.y;
    sd_->qpos[2] = od.pose.pose.position.z;
    // ROS quaternion is xyzw; MuJoCo qpos[3:7] is wxyz.
    double bq[4] = {od.pose.pose.orientation.w, od.pose.pose.orientation.x,
                    od.pose.pose.orientation.y, od.pose.pose.orientation.z};
    mju_normalize4(bq);
    // IMU pitch/roll zero-offset: a constant bias in the perceived vertical makes
    // the planner balance around a false vertical -> a steady lean/creep. Cancel it
    // with a small body-frame post-multiply (wxyz). +pitch tips the perceived torso
    // forward, so the planner leans it back -- counters a forward creep.
    ...
    // free-joint velocity: qvel[0:3] = WORLD linvel, qvel[3:6] = BODY angvel (gyro).
    sd_->qvel[0] = od.twist.twist.linear.x;
    sd_->qvel[1] = od.twist.twist.linear.y;
    sd_->qvel[2] = od.twist.twist.linear.z;
    sd_->qvel[3] = od.twist.twist.angular.x;
    sd_->qvel[4] = od.twist.twist.angular.y;
    sd_->qvel[5] = od.twist.twist.angular.z;
  ...
  void ControlStep() {
    ...
    MaybeReleaseBand(wall, t_ho, od);
    MaybeLogStatus(wall, warming, ls, od, tgt_q, tau, nmotor);
  }

  // Self-contained standing demo: after full-authority policy (past the blend) has
  // kept the torso within band_stable_tilt_ for band_stable_secs continuously, drop
  // the sim's elastic support tether exactly once. No-op on the real robot (the
  // service is absent -> a periodic warning, never a spurious command).
  void MaybeReleaseBand(double wall, double t_ho, const Odometry& od) {
    if (!drop_band_ || band_released_) return;
    if (++band_ticks_ % 20 != 0) return;          // ~10 Hz check, off the 200 Hz hot loop
    if (wall < t_ho + kPolicyBlendSec) return;   // wait for full policy authority
    double R[9]; double bq[4] = {od.pose.pose.orientation.w, od.pose.pose.orientation.x,
                                 od.pose.pose.orientation.y, od.pose.pose.orientation.z};
    mju_normalize4(bq); mju_quat2Mat(R, bq);
    const double tilt = std::acos(std::fmax(-1.0, std::fmin(1.0, R[8])));
    if (tilt > band_stable_tilt_) { stable_since_ = -1.0; return; }  // not upright: reset
    if (stable_since_ < 0.0) { stable_since_ = wall; return; }       // stability window opens
    if (wall - stable_since_ < band_stable_secs_) return;            // not stable long enough
    if (!band_cli_ || !band_cli_->service_is_ready()) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                           "stable %.1fs but /elastic_band/toggle unavailable -- band NOT dropped",
                           band_stable_secs_);
      return;
    }
    ...
    band_released_ = true;
    RCLCPP_INFO(get_logger(),
                "policy stable (tilt<%.1fdeg) for %.1fs at t=%.1fs -> dropping elastic band",
                band_stable_tilt_ * 57.29578, band_stable_secs_, wall);
  }

  // M4: grade torque headroom against TAU_ESTOP (the actual trip point), not TAU_LIMIT.
  void MaybeLogStatus(double wall, bool warming, const LowState& ls, const Odometry& od,
    ...
    RCLCPP_INFO(get_logger(),
                ...
                wall, warming ? "WARMUP" : "policy", od.pose.pose.position.z, tilt,
  ...
  double band_stable_secs_ = 4.0;
  double band_stable_tilt_ = 6.0 * M_PI / 180.0;
  ...
  rclcpp::Subscription<Odometry>::SharedPtr base_sub_;

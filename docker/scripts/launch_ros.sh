...
#
# NOTE: MJPC is NOT built at launch. The mujoco_mpc submodule is bind-mounted
# host-editable and its CMake build tree persists in container_cache/mjpc_build
# (mounted at /home/code/mujoco_mpc/build); (re)build it on demand with
# /home/code/h12_sim_scripts/rebuild_mjpc.sh. h12_deploy_mjpc's CMake links the
# mjpc build tree when present and skips its C++ targets (with a warning) when
# absent, so the colcon build below stays green on a fresh host — run
# rebuild_mjpc.sh once, then `colcon build --packages-select h12_deploy_mjpc`.
set -e

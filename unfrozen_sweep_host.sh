...
robot_fell() { sudo_do docker logs hams_sim_robocasa 2>&1 | grep -q "ROBOT FELL"; }
...
retether_and_verify() {
  # freeze in place (pin captures current pose), reset arm/gripper under the
  # pin, verify, then re-engage the policy and release.
  in_ros "ros2 topic pub --once /hams/freeze_body std_msgs/msg/Bool '{data: true}' >/dev/null 2>&1"
  sleep 2
  sudo_do docker exec hams_ros bash -c "pkill -9 -f lowerbody_controller_node" 2>/dev/null
  sleep 2
  in_ros "ros2 service call /right/gripper/open std_srvs/srv/Trigger >/dev/null 2>&1; sleep 2
ros2 topic pub --once /hams/reset_arm std_msgs/msg/Empty '{}' >/dev/null 2>&1; sleep 3"
  if robot_fell || robot_drifted || ! gripper_ok || ! door_closed; then return 1; fi
  engage_policy
}
...
run_trial() {  # $1=method $2=trial-id
  local M=$1 T=$2
  local EXTRA=""
  [ "$M" != "skill" ] && EXTRA="--max-attempts 20 --no-plan"

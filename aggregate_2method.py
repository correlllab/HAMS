...
overlays — the 2-method study's copy of the campaign's aggregate.py, extended
with the `topdown_irl` method (the live checkout's aggregate.py ignores dirs
it doesn't know). Runs INSIDE hams_ros (needs cv2/numpy). Reads
sweep/<method>/trial_*.json, writes summary.json + per-method overlay PNGs
into <out>/viz/. Differences from the campaign copy: METHOD_ORDER/PRETTY know
topdown_irl; the bar chart / grid lay out however many methods have data
instead of assuming exactly four."""
import glob
import json
import os
import sys
...
SWEEP = sys.argv[1] if len(sys.argv) > 1 else '/home/code/core_ws/benchmark_results/sweep_2method'
TCP_DEPTH_M = 0.1146
METHOD_ORDER = ['centroid', 'topdown_antipodal', 'topdown_irl', 'graspgenx', 'skill']
...
def main():
    ...
    # Combined comparison grid (2 tiles per row) of whichever methods have data.
    present = [m for m in METHOD_ORDER if m in summary]
    tiles = []
    for m in present:
    ...
    if tiles:
        if len(tiles) % 2:
            tiles.append(np.full((512, 512, 3), 30, np.uint8))
        rows = [np.hstack(tiles[i:i + 2]) for i in range(0, len(tiles), 2)]
        cv2.imwrite(os.path.join(viz_dir, 'comparison_grid.png'), np.vstack(rows))

    # Success-rate bar chart, laid out compactly over the methods present.
    x0, y0, w, gap = 60, 320, 130, 30
    bar = np.full((360, x0 * 2 + max(1, len(present)) * (w + gap) - gap, 3), 255, np.uint8)
